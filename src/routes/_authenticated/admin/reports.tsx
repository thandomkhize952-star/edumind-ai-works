...
import { downloadReport, isEmptyDoc, type ExportFormat, type ReportDoc } from "@/lib/report-doc";
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Generate and download filtered reports as CSV or PDF.</p>
        </div>
      </div>
...
function useDownloader<T>(fn: (args: { data: T }) => Promise<any>) {
  ...
  const run = async (input: T, build: (d: any) => { name: string; doc: ReportDoc }) => {
    setBusy(true);
    try {
      const data = await fn({ data: input });
      const { name, doc } = build(data);
      if (isEmptyDoc(doc)) { toast.error("No records matched those filters"); return; }
      await downloadReport(name, format, doc);
  ...
  return { busy, run, format, setFormat };
}
  ...
  return (
    <Card>
      ...
      <CardContent className="grid gap-3 sm:grid-cols-5 sm:items-end">
        ...
        <ExportFormatSelect value={format} onChange={setFormat} />
        <Button
          className="gap-2"
          disabled={busy}
          onClick={() =>
            run({ from: from || null, to: to || null, role }, (d) => ({
              name: `edumind-users-${today()}`,
              doc: {
                title: "EduMind AI — User report",
                meta: [
                ...
                sections: [
                  {
                    head: ["Full name", "Email", "Student number", "Roles", "Registered"],
                    body: d.rows.map((r: any) => [r.full_name, r.email, r.student_number, r.roles.join(" / "), new Date(r.created_at).toLocaleString()]),
                  },
                ],
              },
            }))
        ...
        <Button
          ...
          onClick={() =>
            run({ from: from || null, to: to || null, qualificationId: qual === "all" ? null : qual }, (d) => ({
              name: `edumind-enrollments-${today()}`,
              doc: {
                title: "EduMind AI — Enrollment records",
                meta: [
                  ["Generated at", new Date(d.generated_at).toLocaleString()],
                  ["Date from", from || "any"],
                  ["Date to", to || "any"],
                ...
                sections: [
                  {
                    head: ["Student number", "Full name", "Email", "Qual. code", "Qualification", "Enrolled at"],
                    body: d.rows.map((r: any) => [r.student_number, r.full_name, r.email, r.qualification_code, r.qualification_title, new Date(r.enrolled_at).toLocaleString()]),
                  },
                ],
              },
            }))
        ...
        <Button
          ...
          onClick={() =>
            run({ qualificationId: qual === "all" ? null : qual, lecturerId: lec === "all" ? null : lec }, (d) => ({
              name: `edumind-modules-${today()}`,
              doc: {
                title: "EduMind AI — Module report",
                meta: [["Generated at", new Date(d.generated_at).toLocaleString()]],
                sections: [
                  {
                    head: ["Code", "Title", "Qualification", "Lecturer", "Lecturer email", "Assess.", "Assign.", "Quizzes", "Materials"],
                    body: d.rows.map((r: any) => [r.code, r.title, r.qualification, r.lecturer, r.lecturer_email, r.assessments, r.assignments, r.quizzes, r.materials]),
                  },
                ],
              },
            }))
